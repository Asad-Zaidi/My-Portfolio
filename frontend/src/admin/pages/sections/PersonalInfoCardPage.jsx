...
const SECTION_KEY = "personalInfoCard";
const ITEM_LABEL = "row";

function clone(value) {
  return value === undefined ? value : JSON.parse(JSON.stringify(value));
}

function RowsEditor({ items = [], onChange }) {
  ...
  const add = () => onChange([...items, { icon: "", label: "", value: "", highlight: false }]);
        ...
        <div key={index} className="group flex gap-3 rounded-xl border border-navy-700 bg-navy-800/60 p-4 hover:border-navy-600">
          ...
          <div className="min-w-0 flex-1">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-slate-300">Icon (lucide name)</label>
                <input
                  type="text"
              ...
              <label className="flex items-center gap-2.5 text-sm font-medium text-slate-300 select-none">
                <input
                  ...
                  onChange={(e) => updateItemField(index, "highlight", e.target.checked)}
...
export default function PersonalInfoCardPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Personal Info Card updated.");
    } catch (err) {
  ...
  return (
    <div className="space-y-6 pb-24">
      <div>
        <h1 className="text-xl font-bold text-white">Personal Info Card</h1>
        <p className="mt-1 text-sm text-slate-400">The info table shown next to About (name, DOB, location, etc).</p>
      </div>
      ...
      <div className="grid gap-6 rounded-2xl border border-navy-700 bg-navy-800/50 p-6">
        ...
        <div>
          <div className="mb-2 text-sm font-medium text-slate-300">Rows</div>
          <RowsEditor items={draft.rows || []} onChange={(next) => setField("rows", next)} />
        </div>
      </div>
