...
const SECTION_KEY = "nav";
const ITEM_LABEL = "nav link";

function clone(value) {
  return value === undefined ? value : JSON.parse(JSON.stringify(value));
}

function NavEditor({ items = [], onChange }) {
        ...
        <div key={index} className="group flex gap-3 rounded-xl border border-navy-700 bg-navy-800/60 p-4 hover:border-navy-600">
          ...
          <div className="min-w-0 flex-1">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-slate-300">Anchor (e.g. #about)</label>
                <input
                  type="text"
                  className="w-full rounded-lg border border-navy-600 bg-navy-900/60 px-3.5 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:border-accent focus:ring-2 focus:ring-accent/30"
                  value={item.href || ""}
                  onChange={(e) => updateItemField(index, "href", e.target.value)}
                />
              </div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6 pb-24">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white">Navigation Menu</h1>
          <p className="mt-1 text-sm text-slate-400">The top navbar links, in order.</p>
        </div>
      ...
      <NavEditor items={draft} onChange={setDraft} />
