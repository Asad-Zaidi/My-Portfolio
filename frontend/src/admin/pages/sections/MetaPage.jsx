...
export default function MetaPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("General & SEO updated.");
    } catch (err) {
  ...
  return (
    <div className="space-y-6 pb-24">
      <div>
        <h1 className="text-xl font-bold text-white">General & SEO</h1>
        <p className="mt-1 text-sm text-slate-400">Site title, description and branding used in the browser tab and social previews.</p>
      </div>

      <div className="grid gap-6 rounded-2xl border border-navy-700 bg-navy-800/50 p-6">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-300">Site Title</label>
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-300">Meta Description</label>
          <textarea
            rows={4}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-300">Author</label>
          <input type="text" className="w-full rounded-lg border border-navy-600 bg-navy-900/60 px-3.5 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:border-accent focus:ring-2 focus:ring-accent/30" value={draft.author || ""} onChange={(e) => setField("author", e.target.value)} />
        </div>

        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-300">Theme Color</label>
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-300">Social Share Image (OG Image)</label>
          <ImageUploadField value={draft.ogImage} onChange={(v) => setField("ogImage", v)} />
        </div>
      </div>
