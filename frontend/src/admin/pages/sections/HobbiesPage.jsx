...
const SECTION_KEY = "hobbies";
const ITEM_LABEL = "hobby";

function clone(value) {
  return value === undefined ? value : JSON.parse(JSON.stringify(value));
}

function HobbiesEditor({ items = [], onChange }) {
  ...
  const add = () =>
    onChange([...items, { id: `${SECTION_KEY}-${Date.now().toString(36)}`, name: "", icon: "", description: "" }]);
  ...
  return (
    <div className="space-y-6 pb-24">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white">Hobbies</h1>
          <p className="mt-1 text-sm text-slate-400">Interests outside of work.</p>
        </div>
      ...
      <HobbiesEditor items={draft} onChange={setDraft} />
