...
function LanguagesEditor({ items = [], onChange }) {
  ...
  const add = () => onChange([...items, { name: "", level: "", percent: 0 }]);
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-navy-600 bg-navy-900/60 px-3.5 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:border-accent focus:ring-2 focus:ring-accent/30"
                  value={item.level || ""}
                  onChange={(e) => updateItemField(index, "level", e.target.value)}
                />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-slate-300">Fluency %</label>
                <input
                  type="number"
                  min={0}
                  max={100}
                  ...
                  onChange={(e) => updateItemField(index, "percent", Number(e.target.value))}
                />
              </div>
            </div>
          </div>
...
export default function LanguagesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Spoken Languages updated.");
    } catch (err) {
  ...
  return (
    <div className="space-y-6 pb-24">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white">Spoken Languages</h1>
          <p className="mt-1 text-sm text-slate-400">Languages you speak, with a fluency bar.</p>
        </div>
      ...
      <LanguagesEditor items={draft} onChange={setDraft} />
