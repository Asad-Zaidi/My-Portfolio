...
const SECTION_KEY = "socials";
const ITEM_LABEL = "social link";
...
function SocialsEditor({ items = [], onChange }) {
  ...
  const add = () => onChange([...items, { platform: "", label: "", url: "" }]);
        ...
        <div key={index} className="group flex gap-3 rounded-xl border border-navy-700 bg-navy-800/60 p-4 hover:border-navy-600">
          ...
          <div className="min-w-0 flex-1">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-slate-300">Platform key</label>
                <input
                  type="text"
                  ...
                  value={item.platform || ""}
                  onChange={(e) => updateItemField(index, "platform", e.target.value)}
                />
              </div>
              <div>
    ...
    <div className="space-y-6 pb-24">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Links shown in the navbar footer and hero section.</p>
        </div>
      ...
      <SocialsEditor items={draft} onChange={setDraft} />
